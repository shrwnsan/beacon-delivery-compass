...
class TestPerformanceBenchmarks:
    ...
    def test_collaboration_analyzer_performance(self):
        ...
        assert collaboration_duration < 2.0, f"Collaboration analytics took {collaboration_duration:.2f}s, expected < 2.0s"
    ...
    def test_full_analytics_engine_performance(self):
        ...
        for case in test_cases:
            ...
            assert duration < case["max_time"], f"Analytics for {case['commits']} commits took {duration:.2f}s, expected < {case['max_time']}s"
    ...
    def _create_large_test_repo(self, num_commits: int) -> RangeStats:
        ...
        # Create authors
        authors = [f"Author{i}" for i in range(min(20, num_commits // 10))]  # 1 author per 10 commits, max 20
        if not authors:
            authors = ["Author0"]
        ...
        for i in range(num_commits):
            ...
            commit = CommitStats(
                ...
                date=start_date + timedelta(days=i * 365 // num_commits),  # Evenly distribute over the year
