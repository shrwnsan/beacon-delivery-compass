...
class TestEndToEndPipeline:
    ...
    def _benchmark_repo_size(self, config: dict):
        ...
        # Performance check
        assert analytics_time < config["max_time"], f"Analytics for {config['name']} repo took {analytics_time:.2f}s, expected < {config['max_time']}s"
        ...
        assert total_time < config["max_time"] * 2, f"Full pipeline for {config['name']} repo took {total_time:.2f}s, expected < {config['max_time']*2}s"
