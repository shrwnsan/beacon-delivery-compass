...
class TestPropertyBasedDateParsing:
    ...
    def test_parse_absolute_dates_property(self, date_str: str):
        ...
        try:
            ...
            if ' ' in date_str:
                # Datetime with time
                expected = datetime.strptime(date_str, '%Y-%m-%d %H:%M').replace(tzinfo=timezone.utc)
    ...
    def test_roundtrip_datetime(self, dt: datetime):
        ...
        assert parsed.replace(second=0, microsecond=0) == dt.replace(second=0, microsecond=0)
