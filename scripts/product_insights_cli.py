...
class ProductInsightsCLI:
    ...
    def _has_direct_business_value(self, commit: Dict) -> bool:
        ...
        business_keywords = [
            'revenue', 'conversion', 'sales', 'customer', 'user', 'performance',
            'security', 'scalability', 'feature', 'enhancement'
        ]
        
        has_business_keyword = any(kw in message for kw in business_keywords)
        has_product_component = any(components.get(comp, 0) > 0 
                                  for comp in ['backend', 'frontend', 'api'])
...
def main():
    ...
    parser.add_argument("command", choices=[
        "weekly", "commit", "executive", "alerts"
    ], help="Command to run")
    
    parser.add_argument("--since", default="1 week ago", 
                       help="Start date for analysis")
    parser.add_argument("--commit", default="HEAD", 
                       help="Commit hash for single commit analysis")
