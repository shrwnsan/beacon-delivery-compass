...
class ProductAnalytics:
    ...
    def _is_feature_commit(self, commit: Dict) -> bool:
        ...
        return any(keyword in message for keyword in [
            'feat', 'feature', 'add', 'implement', 'new', 'enhance'
        ])
    ...
    def _is_customer_driven(self, commit: Dict) -> bool:
        ...
        message = commit['commit']['message'].lower()
        return any(keyword in message for keyword in [
            'customer', 'user', 'client', 'request', 'feedback', 'issue', 'bug'
        ])
    ...
    def _is_technical_debt(self, commit: Dict) -> bool:
        ...
        message = commit['commit']['message'].lower()
        return any(keyword in message for keyword in [
            'refactor', 'cleanup', 'optimize', 'performance', 'debt', 'legacy'
        ])
    ...
    def _has_business_impact(self, commit: Dict) -> bool:
        ...
        return (components.get('frontend', 0) > 0 or 
                components.get('api', 0) > 0 or
                components.get('backend', 0) > 0)
    
    def _is_maintenance(self, commit: Dict) -> bool:
        """Determine if commit is maintenance work."""
        components = commit.get('components', {})
        return (components.get('tests', 0) > 0 or
                components.get('documentation', 0) > 0 or
                components.get('configuration', 0) > 0)
...
class CommitClassifier:
    ...
    def _extract_keywords(self, message: str) -> List[str]:
        ...
        keywords = []
        business_keywords = [
            'customer', 'user', 'client', 'revenue', 'conversion', 'engagement',
            'performance', 'security', 'scalability', 'ux', 'ui'
        ]
